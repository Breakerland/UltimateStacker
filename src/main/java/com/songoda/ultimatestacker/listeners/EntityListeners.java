...
public class EntityListeners implements Listener {
	...
	public void onSpawn(CreatureSpawnEvent event) {
		...
		if (event.getSpawnReason().name().equals("DROWNED") || event.getSpawnReason() == CreatureSpawnEvent.SpawnReason.LIGHTNING) {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> plugin.getEntityStackManager().addSerializedStack(entity, name), 1L);
		}
	...
	public void onEgg(ItemSpawnEvent event) {
		...
		if (material != Material.EGG && !material.name().equalsIgnoreCase("SCUTE"))
			return;
		...
		if (!stackManager.isStacked(entity))
			return;
